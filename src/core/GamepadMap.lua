...
--
-- Ported from gen1recomp.  The reason this is a module and not two literal
-- tables in Input.lua: SDL names face buttons by POSITION, and Nintendo's A
-- and B are mirrored relative to the Xbox layout SDL is named after, so on a
-- Switch the button physically labelled A arrives as SDL "b".  Selecting the
-- table at call time keeps that fact in exactly one file.
